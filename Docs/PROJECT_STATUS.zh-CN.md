...
1. 修复现有 6 项测试基线失败，统一关卡顺序、背包容量和装备页面测试数据。
2. 加入对象池、音效、锁定目标、完美闪避、韧性和破防系统。
3. 将关卡改为战斗房、特殊机制房、精英房和奖励选择房的完整流程。
4. 完善主动道具图标、升级分支、三选一奖励和道具组合反应。
5. 角色选择真正切换模型、动画和技能组，并把任务、角色解锁和主动道具槽完整纳入存档。
